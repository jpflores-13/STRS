...
# Date:        2026-04-23
# Project:     STRS
# Description: Two-row supplementary figure. Row 1 (3 panels): HOMER motif
#              QQ scatter plots for CTCF, RAD21, and ATAC loop anchors.
#              Row 2 (3 panels): ENCODE ChIP-seq overlap scatter plots for
#              the same three comparisons. Zoom-in dashed rectangles on each
#              Row 1 panel connect via dotted lines to the corresponding Row 2
#              panel directly below, highlighting the low-enrichment region.
...
## Page layout (inches) — two rows of three equal panels
page_width  <- 10
page_height <- 7.5

panel_w  <- 2.8    ## width of each panel (same for both rows)
panel_h  <- 2.8    ## height of each panel (same for both rows)
row_gap  <- 0.6    ## vertical gap between Row 1 and Row 2 (space for connectors)
...
y_row1 <- top_margin
y_row2 <- top_margin + panel_h + row_gap
...
overlap_list <- lapply(bed_files, function(bf) {
  enc <- tryCatch(
    rtracklayer::import(bf, format = "narrowPeak"),
...
## Helper: build one HOMER QQ ggplot
make_qq <- function(df, x_label, y_label, title) {
  ...
  ggplot(df, aes(x = neg_log10p_x, y = neg_log10p_y)) +
    ...
    ggrepel::geom_text_repel(data = top_motifs, aes(label = motif_short, color = tf_family),
...
## Helper: build one ENCODE scatter ggplot
make_encode <- function(df, x_col, y_col, x_label, y_label, title) {
  ...
  ggplot(df, aes(x = x, y = y)) +
    geom_abline(slope = 1, intercept = 0, color = "grey60",
                linetype = "dashed", linewidth = 0.4) +
    geom_point(data = df_bg, aes(color = factor_class, size = factor_class,
                                 alpha = factor_class)) +
    geom_point(data = df_fg, aes(color = factor_class, size = factor_class,
                                 alpha = factor_class)) +
    ggrepel::geom_text_repel(data = top_points, aes(label = target, color = factor_class),
                             size = 2.2, fontface = "bold", max.overlaps = 25,
                             segment.size = 0.3, segment.color = "grey50",
                             show.legend = FALSE) +
...
p_qq_ctcf  <- make_qq(ctcf_qq_df,  "CTCF Lost",         "CTCF Retained",
                      "CTCF: Retained vs. Lost")
p_qq_rad21 <- make_qq(rad21_qq_df, "RAD21 Lost",        "RAD21 Retained",
                      "RAD21: Retained vs. Lost")
p_qq_atac  <- make_qq(atac_qq_df,  "ATAC Lost anchors", "ATAC Gained anchors",
                      "ATAC: Gained vs. Lost anchors")

## ---- Row 2: ENCODE scatter panels ----
p_enc_ctcf  <- make_encode(overlap_collapsed, "ctcf_lost",   "ctcf_retained",
                           "CTCF lost",   "CTCF retained",
                           "CTCF: Retained vs. Lost")
p_enc_rad21 <- make_encode(overlap_collapsed, "rad21_lost",  "rad21_retained",
                           "RAD21 lost",  "RAD21 retained",
                           "RAD21: Retained vs. Lost")
p_enc_atac  <- make_encode(overlap_collapsed, "atac_lost",   "atac_gained",
                           "ATAC lost",   "ATAC gained",
                           "ATAC: Gained vs. Lost anchors")


# Save outputs ------------------------------------------------------------

## Zoom rectangle geometry — lower-left corner of each Row 1 data area.
## The box marks the dense / low-enrichment region; dotted connectors
## travel through the row_gap whitespace down to the corresponding Row 2 panel.
zoom_frac <- 0.35    ## fraction of data area covered by the zoom box
...
row1_panels <- list(
  list(plot = p_qq_ctcf,  x = x_col[1]),
  list(plot = p_qq_rad21, x = x_col[2]),
  list(plot = p_qq_atac,  x = x_col[3])
)

for (pnl in row1_panels) {
  plotGG(plot = pnl$plot, x = pnl$x, y = y_row1,
         width = panel_w, height = panel_h,
         just = c("left", "top"), default.units = "inches")
}

## ---- Zoom annotations: dashed box on Row 1 + dotted connectors to Row 2 ----
## The data area within each panel is inset ~14% on left and ~8% on top/bottom.
## Connector bottom endpoints are clamped to the top edge of the Row 2 panels.
for (px in x_col) {
  
  data_x0 <- px     + panel_w * 0.14   ## left edge of data area
  data_y0 <- y_row1 + panel_h * 0.08   ## top edge of data area
  data_w  <- panel_w * 0.78
  data_h  <- panel_h * 0.78
  
  ## Zoom box covers the lower-left corner (origin side) of the QQ plot
  box_w <- data_w * zoom_frac
  box_h <- data_h * zoom_frac
  box_x <- data_x0                          ## left-aligned to data area
  box_y <- data_y0 + data_h - box_h         ## bottom-aligned to data area
  ...
  ## Dotted connectors travel straight down through the row_gap into Row 2.
  ## Top endpoints = bottom corners of the zoom box.
  ## Bottom endpoints = corresponding corners at the top of the Row 2 panel.
  row2_top  <- y_row2
  row2_left <- px
  row2_right <- px + panel_w
  
  plotSegments(
    x0 = box_x,          y0 = box_y + box_h,
    x1 = row2_left,      y1 = row2_top,
    lty = 3, lwd = 0.6, linecolor = "grey50",
    default.units = "inches"
  )
  plotSegments(
    x0 = box_x + box_w,  y0 = box_y + box_h,
    x1 = row2_right,     y1 = row2_top,
    lty = 3, lwd = 0.6, linecolor = "grey50",
    default.units = "inches"
  )
}

## ---- Row 2: ENCODE scatter (rendered last — sits on top of connector tips) ----
row2_panels <- list(
  list(plot = p_enc_ctcf,  x = x_col[1]),
  list(plot = p_enc_rad21, x = x_col[2]),
  list(plot = p_enc_atac,  x = x_col[3])
)

for (pnl in row2_panels) {
  plotGG(plot = pnl$plot, x = pnl$x, y = y_row2,
         width = panel_w, height = panel_h,
         just = c("left", "top"), default.units = "inches")
}
