# ##############################################################################
# filename:    cutntag_all_loops_anchor_vs_between_density.R
# author:      JP Flores
# project:     STRS
# date:        2026-04-02
# description: Density plot analysis of CTCF and RAD21 log2FC at loop anchors
#              vs between-anchor regions across all loop categories (gained,
#              lost, static); Wilcoxon tests for each category
# ##############################################################################
...
output_rds        <- "data/processed/cutntag/all_loops_anchor_vs_between_density_analysis.rds"
output_csv        <- "data/processed/cutntag/all_loops_anchor_vs_between_density_analysis.csv"
output_pdf        <- "plots/cutntag_all_loops_anchor_vs_between_density.pdf"
target            <- c("CTCF", "RAD21")
...
# Analysis ----
loop_categories <- list(Gained = gainedLoops, Lost = lostLoops, Static = staticLoops)
results_list    <- list()
...
for (protein in target) {
  ...
  for (category in names(loop_categories)) {
    ...
    anchors_gr <- get_loop_anchors(loops_subset)
    between_gr <- get_between_regions(loops_subset)
...
for (protein in target) {
  protein_data <- all_results |>
    ...
    mutate(loop_category = factor(loop_category, levels = c("Static", "Lost", "Gained")),
  ...
  for (category in c("Static", "Lost", "Gained")) {
    anchor_vals  <- protein_data |> filter(loop_category == category, region_type == "At Anchors")  |> pull(log2FC)
    between_vals <- protein_data |> filter(loop_category == category, region_type == "Between Anchors") |> pull(log2FC)
    if (length(anchor_vals) > 0 && length(between_vals) > 0) {
      cat(category, "loops - Wilcoxon p:", wilcox.test(anchor_vals, between_vals)$p.value, "\n")
    }
  }
  ...
  p <- ggplot(protein_data, aes(x = log2FC, fill = region_type, color = region_type)) +
    ...
    labs(title = paste(protein, "Binding Distribution"),
