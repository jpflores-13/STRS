# ##############################################################################
# filename:    surveyPlot_gainedLoops_HCT116_WT_10kb.R
# author:      JP Flores
# project:     STRS
# date:        2026-04-02
# description: Survey plots for top 100 gained loops in HCT116 WT; two Hi-C
#              rectangles (control / sorbitol 1h) with gained loop annotations
#              and anchor highlights
# ##############################################################################
...
hic_base_path   <- "/work/users/j/p/jpflores/projects/STRS/data/processed/hic/maps"
output_pdf      <- "plots/surveyPlot_gainedLoops_HCT116_WT_10kb.pdf"
n_loops         <- 100L
buffer          <- 200e3
page_width      <- 5.75
page_height     <- 5.5
...
hic_files <- list(
  hct116_control  = file.path(hic_base_path, "STRS_HCT116_WT_control_0h_megaMap_inter_30.hic"),
  hct116_sorbitol = file.path(hic_base_path, "STRS_HCT116_WT_sorbitol_1h_megaMap_inter_30.hic")
)
...
for (i in seq_along(loopRegions_gained_buffed)) {
  ...
  control <- plotHicRectangle(data = hic_files$hct116_control, params = p, y = 0.5)
  ...
  sorb <- plotHicRectangle(data = hic_files$hct116_sorbitol, params = p, y = 2.6)
  ...
  plotText(label = "HCT116 WT control",         x = 0.25, y = 0.5, just = c("top", "left"))
  plotText(label = "HCT116 WT + sorbitol (1h)", x = 0.25, y = 2.6, just = c("top", "left"))
