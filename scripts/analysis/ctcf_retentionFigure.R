...
ctcf_res  <- analyze_by_cpm_bins(ctcf_control_peaks,  ctcf_bam_ctl,  ctcf_bam_trt,  "CTCF",  n_bins = 100)
rad21_res <- analyze_by_cpm_bins(rad21_control_peaks, rad21_bam_ctl, rad21_bam_trt, "RAD21", n_bins = 100)
...
ctcf_summits <- GRanges(
  ...
  ranges   = IRanges(start = summit_positions - flank_size,
...
plot_df_pwm_filtered <- plot_df_pwm |>
  ...
  mutate(pwm_score_filtered = ifelse(
    pwm_score %in% remove_outliers(pwm_score), pwm_score, NA
  )) |>
  ungroup() |>
...
wilcox_result_pwm <- wilcox.test(
  pwm_score_filtered ~ retention_category, data = plot_df_pwm_filtered
)
...
p_panelB <- ggplot(plot_df_pwm_filtered,
  ...
  annotate("segment", x = 1, xend = 2, y = bracket_y_pwm, yend = bracket_y_pwm, linewidth = 0.3) +
...
rad21_data_filtered <- rad21_data |>
  ...
  mutate(rad21_log2fc_filtered = ifelse(
    rad21_log2fc %in% remove_outliers(rad21_log2fc), rad21_log2fc, NA
  )) |>
  ungroup() |>
  filter(!is.na(rad21_log2fc_filtered))

wilcox_test_c <- wilcox.test(
  rad21_log2fc_filtered ~ retention_category, data = rad21_data_filtered
)
...
p_panelC <- ggplot(rad21_data_filtered,
  ...
  annotate("segment", x = 1, xend = 2, y = 0.95, yend = 0.95, linewidth = 0.3, color = "black") +
  annotate("segment", x = 1, xend = 1, y = 0.95, yend = 0.88,  linewidth = 0.3, color = "black") +
  annotate("segment", x = 2, xend = 2, y = 0.95, yend = 0.88,  linewidth = 0.3, color = "black") +
  annotate("text", x = 1.5, y = 1.05, label = sig_label_c, size = 3, color = "black") +
  scale_fill_manual(values = c("Retained" = "#7FCDBB", "Lost" = "#E34A33"),
                    name = "CTCF Peak Status") +
...
mcols(ctcf_control_d)$promoter_category  <- ifelse(promoter_overlaps, "Promoter", "Non-promoter")

cat("Promoter peaks:", sum(promoter_overlaps),
    "(", round(100 * mean(promoter_overlaps), 1), "%)\n")

control_counts  <- bamCount(ctcf_bam_ctl, ctcf_control_d, paired.end = "midpoint")
sorbitol_counts <- bamCount(ctcf_bam_trt, ctcf_control_d, paired.end = "midpoint")

mcols(ctcf_control_d)$control_counts  <- control_counts
mcols(ctcf_control_d)$sorbitol_counts <- sorbitol_counts
mcols(ctcf_control_d)$log2fc          <- log2((sorbitol_counts + 1) / (control_counts + 1))
...
wilcox_result_prom <- wilcox.test(
  log2fc ~ promoter_category, data = peak_data, alternative = "two.sided"
)
...
p_panelD <- ggplot(peak_data,
  ...
  scale_fill_manual(values  = c("Promoter" = "#F8766D", "Non-promoter" = "#619CFF"), name = "") +
  scale_color_manual(values = c("Promoter" = "#F8766D", "Non-promoter" = "#619CFF"), name = "") +
...
plotGG(p_panelA, x = panel_x[1], y = panel_y,
       width = panel_width, height = panel_height, just = c("left", "top"))
plotText(label = "A", x = panel_x[1] - label_offset_x, y = panel_y - label_offset_y,
         fontface = "bold", fontsize = 12)

plotGG(p_panelB, x = panel_x[2], y = panel_y,
       width = panel_width, height = panel_height, just = c("left", "top"))
plotText(label = "B", x = panel_x[2] - label_offset_x, y = panel_y - label_offset_y,
         fontface = "bold", fontsize = 12)

plotGG(p_panelC, x = panel_x[3], y = panel_y,
       width = panel_width, height = panel_height, just = c("left", "top"))
plotText(label = "C", x = panel_x[3] - label_offset_x, y = panel_y - label_offset_y,
         fontface = "bold", fontsize = 12)

plotGG(p_panelD, x = panel_x[4], y = panel_y,
       width = panel_width, height = panel_height, just = c("left", "top"))
plotText(label = "D", x = panel_x[4] - label_offset_x, y = panel_y - label_offset_y,
         fontface = "bold", fontsize = 12)
