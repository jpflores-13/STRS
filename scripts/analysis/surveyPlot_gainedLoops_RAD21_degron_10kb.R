## Gained differential loop visualizations (HiC rectangles) - RAD21 degron
...
## make pdf
pdf(file = "plots/surveyPlot_gainedLoops_RAD21_degron_10kb.pdf",
    width = 5.75,
    height = 5.5)
...
for(i in seq_along(loopRegions_gained_buffed)){
  ...
  ## Plot top Hi-C rectangle (RAD21 sorbitol)
  
  control <- plotHicRectangle(data = hic_files$rad21_sorb,
                              params = p,
                              y = 0.5)
  ...
  ## Plot bottom Hi-C rectangle (RAD21 sorbitol + auxin)
  
  sorb <- 
    plotHicRectangle(data = hic_files$rad21_sorb_auxin, 
                     params = p,
                     y = 2.6)
  ...
  plotText(label = "mAID2-RAD21 + sorbitol",
           x = 0.25,
           y = 0.5,
           just = c("top", "left"))
  
  plotText(label = "mAID2-RAD21 + sorbitol + auxin",
           x = 0.25,
           y = 2.6,
           just = c("top", "left"))
