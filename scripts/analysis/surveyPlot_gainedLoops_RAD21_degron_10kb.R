# ##############################################################################
# filename:    surveyPlot_gainedLoops_RAD21_degron_10kb.R
# author:      JP Flores
# project:     STRS
# date:        2026-04-02
# description: Survey plots for top 100 gained loops in mAID2-RAD21 degron
#              experiment; two Hi-C rectangles (sorbitol / sorbitol + auxin)
#              with gained loop annotations and anchor highlights
# ##############################################################################
...
output_pdf     <- "plots/surveyPlot_gainedLoops_RAD21_degron_10kb.pdf"
n_loops        <- 100L
buffer         <- 200e3
page_width     <- 5.75
page_height    <- 5.5
...
hic_files <- list(
  rad21_sorb       = file.path(hic_base_path, "YAPP_HCT116_mAID2-RAD21_sorbitol",
                                "YAPP_HCT116_mAID2-RAD21_sorbitol_inter_30.hic"),
  rad21_sorb_auxin = file.path(hic_base_path, "YAPP_HCT116_mAID2-RAD21_sorbitol_auxin",
                                "YAPP_HCT116_mAID2-RAD21_sorbitol_auxin_inter_30.hic")
)
...
for (i in seq_along(loopRegions_gained_buffed)) {
  ...
  control <- plotHicRectangle(data = hic_files$rad21_sorb, params = p, y = 0.5)
  ...
  sorb <- plotHicRectangle(data = hic_files$rad21_sorb_auxin, params = p, y = 2.6)
  ...
  plotText(label = "mAID2-RAD21 + sorbitol",         x = 0.25, y = 0.5, just = c("top", "left"))
  plotText(label = "mAID2-RAD21 + sorbitol + auxin", x = 0.25, y = 2.6, just = c("top", "left"))
