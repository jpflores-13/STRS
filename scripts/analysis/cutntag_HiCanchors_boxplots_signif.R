# ##############################################################################
# filename:    cutntag_HiCanchors_boxplots_signif.R
# author:      JP Flores
# project:     STRS
# date:        2026-04-02
# description: Boxplots of CUT&Tag log2FC at gained, lost, and static loop
#              anchors for CTCF, H3K27ac, RAD21, and YAP1; individual and
...
output_pdf        <- "plots/cutntag_peak_HiCanchors_individual_plots.pdf"
...
for (i in seq_along(cutntag)) {
  peak_list[[peak_names[i]]] <- plyranges::read_narrowpeaks(cutntag[i])
}
...
cat("\nAnchor counts:\n")
cat("Gained:", length(gained_anchors), "\n")
cat("Lost:",   length(lost_anchors),   "\n")
cat("Static:", length(static_anchors), "\n")

results <- data.frame()

for (t in target) {
  if (!paste0(t, "_control") %in% names(peak_list)) next

  control_peaks <- peak_list[[paste0(t, "_control")]]
  if (length(control_peaks) == 0) next

  if (!paste0(t, "_control") %in% names(bam_files) ||
      !paste0(t, "_sorbitol") %in% names(bam_files)) next
  ...
  if (length(overlapping_peaks) == 0) next

  peak_lfc <- calculate_peak_lfc(overlapping_peaks, control_bam, treatment_bam)

  peak_ids <- if (is.null(names(overlapping_peaks)) || all(names(overlapping_peaks) == "")) {
    paste0(as.character(seqnames(overlapping_peaks)), ":",
           start(overlapping_peaks), "-", end(overlapping_peaks))
  } else {
    names(overlapping_peaks)
  }
  ...
  target_df <- data.frame(
    ...
    loop_type  = mcols(all_anchors)[match(mcols(overlapping_peaks)$anchor_id,
                                         mcols(all_anchors)$loop_id), "loop_type"],
    log2FC     = peak_lfc,
...
facet_labels <- c("CTCF" = "CTCF", "H3K27ac" = "H3K27ac", "RAD21" = "RAD21", "YAP1" = "YAP1")

create_protein_plot <- function(data, protein_name) {
  ggplot(data, aes(x = fct_rev(loop_type), y = log2FC, fill = loop_type)) +
    ...
    scale_fill_manual(values = c("Gained" = "#E64B35", "Lost" = "#4DBBD5", "Static" = "#999999")) +
    labs(x = "", y = "log2FoldChange(Treated/Control)", title = protein_name) +
...
summary_stats <- results |>
  ...
  summarize(mean_lfc   = mean(log2FC, na.rm = TRUE),
