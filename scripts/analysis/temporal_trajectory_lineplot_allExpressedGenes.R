# ##############################################################################
# filename:    temporal_trajectory_lineplot_allExpressedGenes.R
# author:      JP Flores
# project:     STRS
# date:        2026-04-02
# description: Median RNA-seq log2FC trajectory for genes at gained, lost, and
#              static loop anchors; uses ALL expressed genes (no LRT filter);
#              one-sample t-test annotations on gained loop genes
# ##############################################################################
...
output_pdf     <- "plots/temporal_trajectory_lineplot_allExpressedGenes.pdf"
...
label_data <- combined |>
  ...
  summarise(x     = 24,
            ...
            .groups = "drop") |>
  mutate(x_adjusted = x + 0.5, y_adjusted = y - 0.01)
...
line_plot <- ggplot(combined,
  ...
  scale_y_continuous(breaks = seq(-0.125, 0.125, 0.0625),
                     labels = c("-0.125", "", "0", "", "0.125"),
                     minor_breaks = NULL) +
  coord_cartesian(xlim = c(1, 28), ylim = c(-0.125, 0.125)) +
