...
##make pdf
pdf(file = "plots/exampleRegion_lost.pdf",
    width = 5.75,
    height = 5.75)
...
## Loop through each region
  ...
  annoHighlight(
    plot = control,
    fill = "lightgrey",
    chrom = as.character(seqnames(loopRegions_lost))[20],
    chromstart = start(loopRegions_lost)[20],
    chromend = start(loopRegions_lost)[20] + 10e3,
    default.units = "inches",
  ...
  annoHighlight(
    ...
    fill = "lightgrey",
    chrom = as.character(seqnames(loopRegions_lost))[20],
    chromstart = end(loopRegions_lost)[20],
    chromend = end(loopRegions_lost)[20] - 10e3,
    default.units = "inches"
  )
