## Density plot analysis: Anchors vs Between regions for GAINED loops only
...
for (protein in target) {
  ...
  anchors <- get_loop_anchors(gainedLoops)
  between <- get_between_regions(gainedLoops)
  ...
  results_list[[paste0(protein, "_Gained")]] <- combined_results
...
## Save results
saveRDS(all_results, "data/processed/cutntag/gained_loops_anchor_vs_between_density_analysis.rds")
write.csv(all_results, "data/processed/cutntag/gained_loops_anchor_vs_between_density_analysis.csv", 
          row.names = FALSE)

# Visualization -----------------------------------------------------------

pdf("plots/cutntag_gained_loops_anchor_vs_between_density.pdf", width = 8, height = 5)
...
for (protein in target) {
  ...
  anchor_vals <- protein_data |> 
    filter(region_type == "At Anchors") |> 
    pull(log2FC)
  between_vals <- protein_data |> 
    filter(region_type == "Between Anchors") |> 
    pull(log2FC)
  
  if (length(anchor_vals) > 0 && length(between_vals) > 0) {
    test_result <- wilcox.test(anchor_vals, between_vals)
    cat("Gained loops - Wilcoxon test p-value:", test_result$p.value, "\n")
  }
  ...
  p <- ggplot(protein_data, aes(x = log2FC, fill = region_type, color = region_type)) +
    ...
    labs(
      title = paste(protein, "Binding Distribution - Gained Loops"),
...
cat("\n\nAnalysis complete!\n")
cat("Plots saved to: plots/cutntag_gained_loops_anchor_vs_between_density.pdf\n")
